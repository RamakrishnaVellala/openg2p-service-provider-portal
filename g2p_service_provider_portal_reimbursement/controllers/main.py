...
from odoo.addons.g2p_portal_base.controllers.main import PortalBaseController

_logger = logging.getLogger(__name__)


class ServiceProviderController(PortalBaseController):
    @http.route(["/portal/serviceprovider/reimbursement/voucher"], type="http", auth="user", website=True)
    def portal_new_entitlements(self, **kwargs):
        ...
        return request.render(
            "g2p_service_provider_portal_reimbursement.reimbursements",
            {
                "entitlements": values,
            },
        )
    ...
    @http.route(
        ["/portal/serviceprovider/reimbursement/voucher/<int:_id>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_new_submission(self, _id, **kwargs):
        ...
        if len(entitlement.reimbursement_entitlement_ids) > 0:
            return request.redirect(f"/portal/serviceprovider/reimbursement/claim/{_id}")
    ...
    @http.route(
        ["/portal/serviceprovider/reimbursement/submit/<int:_id>"],
        type="http",
        auth="user",
        website=True,
        csrf=False,
    ...
    def portal_claim_submission(self, _id, **kwargs):
        ...
        if request.httprequest.method == "POST":
            ...
            if reimbursement_claim == (2, None):
                ...
                return request.redirect(f"/portal/serviceprovider/reimbursement/voucher/{_id}")
            ...
            if len(entitlement.reimbursement_entitlement_ids) == 0:
                return request.redirect(f"/portal/serviceprovider/reimbursement/entitlement/{_id}")

        return request.redirect(f"/portal/serviceprovider/reimbursement/claim/{_id}")

    @http.route(
        ["/portal/serviceprovider/reimbursement/claim/<int:_id>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_post_submission(self, _id, **kwargs):
        ...
        return request.render(
            "g2p_service_provider_portal_reimbursement.reimbursement_form_submitted",
            {
    ...
    @http.route(
        "/portal/serviceprovider/reimbursement/get_voucher_codes", type="http", auth="user", website=True
    )
